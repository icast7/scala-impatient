...
class PersonFullName(val fullName: String){
  private val nameComponents = fullName.split(' ').filter(_.nonEmpty)
  require(nameComponents.size == 2)
  val firstName: String = nameComponents(0)
  val lastName: String = nameComponents(1)
}
val personFullName = new PersonFullName(" Fred Smith")
personFullName.firstName
personFullName.lastName
//A val as the properties are required to be read-only
